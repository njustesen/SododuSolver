...
public class SudokuSolver implements ISudokuSolver {
	...
	/* Sets up the solve by creating the puzzle and initializing the domains. */
	public void setup(int size1) {
		...
		for (int i = 0; i < vars; i++){
			D.add(getSudokuDomain());
		}
		
	}

	/* Returns a domain for a variable in sudoku */
	private ArrayList<Integer> getSudokuDomain() {
	 ...
	 * Return true of the puzzle was solved and saved, false if no solution exists */
	...
	private void setPuzzle(ArrayList<Integer> solution) {
		...
		public ArrayList<Integer> FC(ArrayList<Integer> asn) {
			// If complete, return the assignent
			if (isComplete(asn)){
				return asn;
			}
			
			// Find first zero in assignement
			int x = getIndexOfFirstZero(asn);
			
			// Clone and save old domain
			ArrayList<ArrayList<Integer>> oldDomain = cloneDomain(D);

			// Iterate all values in domain
			for(int i = 0; i < oldDomain.get(x).size(); i++){
				...
				if (AC_FC(x,v)){
					...
					// If a solution was found return the assignment
					if (R != null){
						return R;
					}
					// Roll back
					asn.set(x, 0);
					D = cloneDomain(oldDomain);
				} else {
					D = cloneDomain(oldDomain);
				}
			}
		...
		/* Clones a domain and its objects. */ 
		private ArrayList<ArrayList<Integer>> cloneDomain(ArrayList<ArrayList<Integer>> asn) {
			...
			for(int o = 0; o < asn.size(); o++){
				...
				// Add new objects to the new domain
		...
		/* Returns the first zero in the assignemnt */ 
		...
		/* Returns true the assignment is complete */
